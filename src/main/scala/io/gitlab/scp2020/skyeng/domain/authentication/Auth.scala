...
object Auth {
  ...
  private def _teacherOnly[F[_], Auth](implicit
      F: MonadError[F, Throwable]
  ): BasicRBAC[F, Role, User, Auth] =
    BasicRBAC[F, Role, User, Auth](Role.Admin, Role.Teacher)

  def teacherOnly[F[_], Auth](
      pf: PartialFunction[SecuredRequest[F, User, AugmentedJWT[Auth, Long]], F[
        Response[F]
      ]]
  )(implicit
    ...
    TSecAuthService.withAuthorization(
      _teacherOnly[F, AugmentedJWT[Auth, Long]]
    )(
      pf
    )
}
