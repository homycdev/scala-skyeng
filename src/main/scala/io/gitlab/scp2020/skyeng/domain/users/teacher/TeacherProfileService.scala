...
class TeacherProfileService[F[_]](teacherRepo: TeacherRepositoryAlgebra[F], validation: TeacherValidationAlgebra[F]) {
  def createTeacher(teacherProfile: TeacherProfile)(implicit M: Monad[F]): EitherT[F, TeacherAlreadyExistsError, TeacherProfile] =
    for {
      _ <- validation.teacherDoesNotExist(teacherProfile)
      saved <- EitherT.liftF(teacherRepo.create(teacherProfile))
    } yield saved

  def getTeacher(teacherId: Long)(implicit F: Functor[F]): EitherT[F, TeacherNotFoundError.type, TeacherProfile] =
  ...
  def updateTeacher(teacher: TeacherProfile)(implicit M: Monad[F]): EitherT[F, TeacherNotFoundError.type, TeacherProfile] =
    for {
...
object TeacherProfileService {
  def apply[F[_]](
                   teacherRepositoryAlgebra: TeacherRepositoryAlgebra[F],
                   validationAlgebra: TeacherValidationAlgebra[F],
